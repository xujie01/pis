...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('pis-upms-systemid', systemid);
		$.cookie('pis-upms-systemname', systemname);
		$.cookie('pis-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('pis-upms-systemid') || 1;
	var systemname = $.cookie('pis-upms-systemname') || 'pis-upms-server';
	var systemtitle = $.cookie('pis-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
